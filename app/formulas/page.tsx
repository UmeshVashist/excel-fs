...
function FormulasContent() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      setUser(currentUser)
      loadFormulas(currentUser.id)
    }
    checkUser()
  }, [router])

  const loadFormulas = async (userId: string) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      ...
      <FormulaForm open={isFormOpen} onOpenChange={handleFormClose} formula={editingFormula} userId={user.id} />
    </div>
  )
}
